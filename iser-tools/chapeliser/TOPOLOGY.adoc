...
distributes any workload across Chapel clusters without the user writing
Chapel code. It reads a `+chapeliser.toml+` manifest describing entry
points, data partition strategies, and gather strategies, then generates
Chapel wrapper code, a Zig FFI bridge, and C headers. chapeliser is
priority #2 in the -iser family, targeting any compute workload that
benefits from data-parallel or distributed execution.
