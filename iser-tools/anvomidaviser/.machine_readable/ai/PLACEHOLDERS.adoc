...
sed -i 's/anvomidaviser/my-project/g' $(grep -rl 'anvomidaviser' .)
sed -i 's/Anvomidaviser/MY_PROJECT/g' $(grep -rl 'Anvomidaviser' .)
sed -i 's/anvomidaviser/my_project/g' $(grep -rl 'anvomidaviser' .)
sed -i 's/anvomidaviser/my-project/g' $(grep -rl 'anvomidaviser' .)
sed -i 's/github.com/github.com/g' $(grep -rl 'github.com' .)
sed -i "s/2026/$(date +%Y)/g" $(grep -rl '2026' .)
sed -i "s/2026-03-21/$(date +%Y-%m-%d)/g" $(grep -rl '2026-03-21' .)
```
...
| `JDAJ` | Author initials (for citations) | `J.` | docs/CITATIONS.adoc |

### Project Identity

| Placeholder | Description | Example | Files |
|---|---|---|---|
| `anvomidaviser` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `Convert ISU notation to formal figure skating programs via Anvomidav` | One-line description | `A tool for X` | flake.nix |
| `Anvomidaviser` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `anvomidaviser` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `anvomidaviser` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `2026-03-21` | Current date (ISO) | `2026-02-14` | STATE.a2ml, MAINTAINERS.md |
| `2026-03-21` | Last updated date | `2026-02-14` | TOPOLOGY.md, THREAT-MODEL.md |
...
| `https://github.com/hyperpolymath/anvomidaviser` | Project website | `https://example.org` | SECURITY.md |
| `j.d.a.jewell@open.ac.uk` | Conduct reports email | `conduct@example.org` | CODE_OF_CONDUCT.md |
| `hyperpolymath` | Conduct committee name | `Code of Conduct Committee` | CODE_OF_CONDUCT.md |
...
| `Convert ISU notation to formal figure skating programs` | One-line project description | `FFI bridges between languages` | STATE.a2ml |

### AI Manifest

| Placeholder | Description | Example | Files |
|---|---|---|---|
| `anvomidaviser` | Repository name | `my-project` | 0-AI-MANIFEST.a2ml |
