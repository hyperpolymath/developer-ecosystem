...
Complete reference for asdf-ghjk functions, scripts, and environment
variables.

=== Table of Contents

* link:#scripts[Scripts]
* link:#library-functions[Library Functions]
* link:#environment-variables[Environment Variables]
* link:#exit-codes[Exit Codes]
* link:#file-formats[File Formats]
...
*Environment Variables*: - `+GITHUB_API_TOKEN+` (optional): GitHub API
token for higher rate limits

*Exit Codes*: - `+0+`: Success - `+1+`: GitHub API error, network error,
or no versions found

*Performance*: O(n) where n = number of releases; ~1-3 seconds without
cache, <100ms with cache
...
*Required Environment Variables*: - `+ASDF_INSTALL_VERSION+`: Version to
download (e.g., "`0.3.2`") - `+ASDF_DOWNLOAD_PATH+`: Where to download
files

*Optional Environment Variables*: - `+GITHUB_API_TOKEN+`: GitHub API
token

*Side Effects*: - Downloads archive to
`+${ASDF_DOWNLOAD_PATH}/<asset-name>.tar.gz+` - Creates `+.metadata+`
file with version info

*Exit Codes*: - `+0+`: Success - `+1+`: Missing environment variable,
...
*Required Environment Variables*: - `+ASDF_INSTALL_VERSION+`: Version to
install - `+ASDF_INSTALL_PATH+`: Where to install -
`+ASDF_INSTALL_TYPE+`: Must be "`version`"

*Optional Environment Variables*: - `+ASDF_DOWNLOAD_PATH+`: Where files
were downloaded (defaults to adjacent to install path)

*Side Effects*: - Extracts files to `+${ASDF_INSTALL_PATH}/+` - Creates
`+${ASDF_INSTALL_PATH}/bin/+` directory - Sets executable permissions -
Creates symlinks if needed

*Exit Codes*: - `+0+`: Success - `+1+`: Missing environment variable,
extraction failure, or binary not found
...
*Exit Codes*: - `+0+`: Success - `+1+`: No install path provided
...
*Exit Codes*: - `+0+`: Success - `+1+`: No stable versions found or
GitHub API error
...
*Possible Values*: - `+x86_64-unknown-linux-gnu+` -
`+aarch64-unknown-linux-gnu+` - `+x86_64-apple-darwin+` -
`+aarch64-apple-darwin+`

*Exit Codes*: - `+0+`: Success - `+1+`: Unsupported platform
...
*Exit Codes*: - `+0+`: Command exists - `+1+`: Command not found
...
*Exit Codes*: - `+0+`: All dependencies found - `+1+`: One or more
...
*Environment Variables Used*: - `+GITHUB_API_TOKEN+` (optional)
...
*Exit Codes*: - `+0+`: Success - `+1+`: API error or rate limit exceeded
...
*Arguments*: - `+$1+`: Version (with or without '`v`' prefix) - `+$2+`:
Platform string
...
*Arguments*: - `+$1+`: Version - `+$2+`: Platform string
...
*Arguments*: - `+$1+`: URL to download - `+$2+`: Output file path
...
*Exit Codes*: - `+0+`: Success - `+1+`: Failed after retries
...
*Arguments*: - `+$1+`: File to verify - `+$2+`: Expected SHA256 hash
...
*Exit Codes*: - `+0+`: Checksum matches or no checksum provided - `+1+`:
Checksum mismatch
...
*Arguments*: - `+$1+`: Archive file path - `+$2+`: Destination directory
...
*Arguments*: - `+$1+`: URL that was cached

*Returns*: Cached response or nothing

*Exit Codes*: - `+0+`: Valid cache found - `+1+`: No cache or expired
...
*Arguments*: - `+$1+`: URL to cache - `+$2+`: Response data
...
|`+GITHUB_API_TOKEN+` |GitHub API authentication |None
|`+ghp_abc123...+`
...
|`+Error: GitHub API rate limit+` |Too many requests |Set
`+GITHUB_API_TOKEN+`

|`+Error: Checksum verification failed+` |Download corrupted
|Re-download

|`+Error: Unsupported platform+` |Platform not supported |Check
compatibility
|===
...
Scripts follow semantic versioning conceptually but are versioned with
the plugin.

==== API Stability

*Stable* (will not break): - All `+bin/*+` scripts interface with asdf -
Environment variables read/written - Exit codes - File formats

*Internal* (may change): - Library function signatures - Internal
variable names - Cache format - Log message formats
...
[arabic]
. *Use caching*: Keep `+GHJK_CACHE_TTL+` at default or higher
. *Set GitHub token*: Avoid rate limits
. *Parallel installs*: Install multiple versions in separate shells
. *Clean cache*: Run `+./scripts/cleanup.sh --cache+` periodically
