...
set -euo pipefail
readonly REPO="restic/restic"
export TOOL_NAME="restic" TOOL_CMD="restic"
log_info() { echo "[asdf-restic] $*" >&2; }
fail() { echo "[asdf-restic] ERROR: $*" >&2; exit 1; }
...
get_download_url() { local v="$1" p a; p="$(get_platform)"; a="$(get_arch)"; echo "https://github.com/${REPO}/releases/download/v${v}/restic_${v}_${p}_${a}.bz2"; }
check_dependencies() { command -v curl &>/dev/null || fail "curl required"; command -v bzip2 &>/dev/null || fail "bzip2 required"; }
