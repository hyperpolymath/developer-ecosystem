...
set -euo pipefail
readonly REPO="gitleaks/gitleaks"
export TOOL_NAME="gitleaks" TOOL_CMD="gitleaks"
log_info() { echo "[asdf-gitleaks] $*" >&2; }
fail() { echo "[asdf-gitleaks] ERROR: $*" >&2; exit 1; }
get_platform() { case "$(uname -s)" in Linux*) echo "linux" ;; Darwin*) echo "darwin" ;; *) fail "Unsupported" ;; esac; }
get_arch() { case "$(uname -m)" in x86_64|amd64) echo "x64" ;; aarch64|arm64) echo "arm64" ;; *) fail "Unsupported" ;; esac; }
...
get_download_url() { local v="$1" p a; p="$(get_platform)"; a="$(get_arch)"; echo "https://github.com/${REPO}/releases/download/v${v}/gitleaks_${v}_${p}_${a}.tar.gz"; }
