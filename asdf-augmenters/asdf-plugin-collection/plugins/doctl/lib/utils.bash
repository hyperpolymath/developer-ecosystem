...
set -euo pipefail
readonly REPO="digitalocean/doctl"
export TOOL_NAME="doctl" TOOL_CMD="doctl"
log_info() { echo "[asdf-doctl] $*" >&2; }
fail() { echo "[asdf-doctl] ERROR: $*" >&2; exit 1; }
get_platform() { case "$(uname -s)" in Linux*) echo "linux" ;; Darwin*) echo "darwin" ;; *) fail "Unsupported" ;; esac; }
get_arch() { case "$(uname -m)" in x86_64) echo "amd64" ;; aarch64|arm64) echo "arm64" ;; *) fail "Unsupported" ;; esac; }
...
get_download_url() { local v="${1}" p="$(get_platform)" a="$(get_arch)"; echo "https://github.com/${REPO}/releases/download/v${v}/doctl-${v}-${p}-${a}.tar.gz"; }
