...
let requireParam = (paramName: string): guard => url => {
  let params = Tea_QueryParams.parse(url.query->Option.getOr(""))
  if Tea_QueryParams.has(params, paramName) {
    Allow
  } else {
...
let guardedPush = (config: guardConfig, url: Tea_Url.t): bool => {
  ...
  | Redirect(path) => {
      ...
      Tea_Navigation.execute(
        Tea_Navigation.Replace(Tea_Url.parse(path))
      )
      true
    }
  }
...
let guardedPushAsync = async (config: guardConfig, url: Tea_Url.t): promise<bool> => {
