...
*Objective:* Bundle AffineScript compilation elements into games while
maintaining clear AGPL licensing for game content and PMPL licensing for
core technology.

*Key Principle:* _AffineScript is required to change the game, but not
to run it._
...
*Runtime Requirements (Players):* - ✅ game.wasm (AGPL) - ✅ gossamer
runtime (PMPL) - ✅ burble client (PMPL) - ❌ affinescript compiler (not
needed)

*Development Requirements (Modders):* - ✅ All runtime components - ✅
affinescript compiler (PMPL) - ✅ stdlib and tools (PMPL)
...
[arabic]
. *README.adoc* still shows PMPL licensing for everything
. *No clear separation* between game content and technology
. *AGPL-3.0* file exists but not referenced for game content
. *Dual licensing* not clearly explained
...
. *Modification Sharing:* Changes must be open
. *Network Use:* Source must be available for network access
. *License Preservation:* AGPL license must be included
. *Copyright Notices:* All notices must be preserved

==== PMPL-1.0 Requirements

[arabic]
. *License Preservation:* PMPL license must be included
. *Copyright Notices:* All notices must be preserved
. *Modification Documentation:* Changes must be documented
. *Ethical Use:* Follow ethical guidelines
. *No Copyleft:* No requirement to open source derived works
...
[arabic]
. *Dual Licensing:* AGPL for games, PMPL for technology
. *Clear Separation:* Game content vs core tools
. *Stapeln Pods:* Developer vs player distributions
. *Compliance:* Separate license files and documentation
. *Future-Proof:* Designed for game distribution
...
*Result:* Clear AGPL licensing for game content with PMPL licensing for
core technology, enabling open game development while maintaining
permissive tooling licenses.
