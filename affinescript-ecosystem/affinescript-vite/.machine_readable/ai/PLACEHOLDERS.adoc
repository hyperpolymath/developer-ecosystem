...
sed -i 's/AffineScript-Vite/my-project/g' $(grep -rl 'AffineScript-Vite' .)
sed -i 's/DEVELOPER_ECOSYSTEM/MY_PROJECT/g' $(grep -rl 'DEVELOPER_ECOSYSTEM' .)
sed -i 's/affinescript-vite/my_project/g' $(grep -rl 'affinescript-vite' .)
sed -i 's/affinescript-vite/my-project/g' $(grep -rl 'affinescript-vite' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `AffineScript-Vite` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `{{PROJECT_DESCRIPTION}}` | One-line description | `A tool for X` | flake.nix |
| `DEVELOPER_ECOSYSTEM` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `affinescript-vite` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `affinescript-vite` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `https://github.com/hyperpolymath/affinescript-vite` | Project website | `https://example.org` | SECURITY.md |
