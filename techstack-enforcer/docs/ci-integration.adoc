...
Integrate Techstack Enforcer into your CI/CD pipelines to automatically
enforce techstack policies.
...
[arabic]
. Copy `+ci-templates/github-action-techstack.yml+` to
`+.github/workflows/techstack.yml+`
. Set repository variable `+TECHSTACK_REPO+`:
* Go to Settings > Secrets and variables > Actions > Variables
* Add: `+TECHSTACK_REPO+` = `+hyperpolymath/techstack-enforcer+`
. Customize definition set and mode in the workflow file
...
[arabic]
. *Start with `+permissive+`* - Begin in advisory mode to understand
violations
. *Graduate to `+moderate+`* - Block high-risk patterns
. *Use `+strict+` for security-critical* - Maximum enforcement for
sensitive code
. *Run learning mode weekly* - Discover new patterns
. *Check changed files on PRs* - Faster feedback for developers
. *Full audit on main branch* - Comprehensive coverage
