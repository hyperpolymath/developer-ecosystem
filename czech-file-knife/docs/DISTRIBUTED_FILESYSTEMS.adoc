...
Czech File Knife (CFK) provides unified access to various distributed
and network filesystems through its provider abstraction layer.
...
* *Feature*: `+s3+`
* *Providers*: AWS, MinIO, Cloudflare R2, Backblaze B2, DigitalOcean
Spaces, Wasabi
...
¹ S3 rename is copy+delete ² S3 copy is server-side for same bucket ³
SMB Previous Versions ⁴ IPFS content is immutable; IPNS provides
mutability
...
CFK automatically uses the caching layer (`+cfk-cache+`) to optimize
performance:

[arabic]
. *Metadata Cache*: TTL-based caching of file/directory metadata
. *Content Cache*: Content-addressed blob storage with LZ4 compression
. *Eviction Policies*: LRU, LFU, FIFO, or adaptive policies
