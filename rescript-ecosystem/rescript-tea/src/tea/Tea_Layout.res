...
let flexContainerStyle = (config: flexConfig): ReactDOM.Style.t => {
  let style = ReactDOM.Style.make(~display="flex", ())

  let style = switch config.direction {
  | Some(dir) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~flexDirection=flexDirectionToString(dir), ()))
  | None => style
  }

  let style = switch config.wrap {
  | Some(wrap) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~flexWrap=flexWrapToString(wrap), ()))
  | None => style
  }

  let style = switch config.justifyContent {
  | Some(jc) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~justifyContent=justifyContentToString(jc), ()))
  | None => style
  }

  let style = switch config.alignItems {
  | Some(ai) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~alignItems=alignItemsToString(ai), ()))
  | None => style
  }

  let style = switch config.alignContent {
  | Some(ac) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~alignContent=alignContentToString(ac), ()))
  | None => style
  }

  let style = switch config.gap {
  | Some(gap) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gap=lengthToString(gap), ()))
  | None => style
  }

  let style = switch config.rowGap {
  | Some(gap) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~rowGap=lengthToString(gap), ()))
  | None => style
  }

  let style = switch config.columnGap {
  | Some(gap) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~columnGap=lengthToString(gap), ()))
  | None => style
  }
...
let flexItemStyle = (config: flexItemConfig): ReactDOM.Style.t => {
  let style = ReactDOM.Style.make()

  let style = switch config.grow {
  | Some(grow) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~flexGrow=Belt.Float.toString(grow), ()))
  | None => style
  }

  let style = switch config.shrink {
  | Some(shrink) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~flexShrink=Belt.Float.toString(shrink), ()))
  | None => style
  }

  let style = switch config.basis {
  | Some(basis) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~flexBasis=lengthToString(basis), ()))
  | None => style
  }

  let style = switch config.alignSelf {
  | Some(align) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~alignSelf=alignItemsToString(align), ()))
  | None => style
  }

  let style = switch config.order {
  | Some(order) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~order=Belt.Int.toString(order), ()))
  | None => style
  }
...
let gridContainerStyle = (config: gridConfig): ReactDOM.Style.t => {
  let style = ReactDOM.Style.make(~display="grid", ())

  let style = switch config.templateColumns {
  | Some(cols) =>
    let colStr = cols->Belt.Array.map(trackToString)->Belt.Array.joinWith(" ", x => x)
    ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridTemplateColumns=colStr, ()))
  | None => style
  }
  ...
  | Some(rows) =>
    ...
    ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridTemplateRows=rowStr, ()))
  | None => style
  }
  ...
  | Some(areas) =>
    ...
    ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridTemplateAreas=areasStr, ()))
  | None => style
  }
  ...
  | Some(ji) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~justifyItems=justifyContentToString(ji), ()))
  | None => style
  }
  ...
  | Some(flow) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridAutoFlow=flow, ()))
  | None => style
  }

  let style = switch config.autoRows {
  | Some(rows) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridAutoRows=lengthToString(rows), ()))
  | None => style
  }

  let style = switch config.autoColumns {
  | Some(cols) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridAutoColumns=lengthToString(cols), ()))
  | None => style
  }
...
let gridItemStyle = (config: gridItemConfig): ReactDOM.Style.t => {
  let style = ReactDOM.Style.make()

  let style = switch config.placement {
  | Some({rowStart, rowEnd, columnStart, columnEnd}) =>
    let style = switch rowStart {
    | Some(line) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridRowStart=gridLineToString(line), ()))
    | None => style
    }
    let style = switch rowEnd {
    | Some(line) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridRowEnd=gridLineToString(line), ()))
    | None => style
    }
    let style = switch columnStart {
    | Some(line) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridColumnStart=gridLineToString(line), ()))
    | None => style
    }
    let style = switch columnEnd {
    | Some(line) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridColumnEnd=gridLineToString(line), ()))
    | None => style
    }
    style
  | None => style
  ...
  | Some(area) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~gridArea=area, ()))
  | None => style
  }

  let style = switch config.justifySelf {
  | Some(js) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~justifySelf=justifyContentToString(js), ()))
  | None => style
  }

  let style = switch config.alignSelf {
  | Some(as_) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~alignSelf=alignItemsToString(as_), ()))
  | None => style
  }
...
let quadrantContainerStyle = (config: quadrantConfig): ReactDOM.Style.t => {
  ...
  let style = switch config.gap {
  | Some(gap) => ReactDOM.Style.combine(baseStyle, ReactDOM.Style.make(~gap=lengthToString(gap), ()))
  | None => baseStyle
  }

  let style = switch config.minCellHeight {
  | Some(height) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~minHeight=`calc(${lengthToString(height)} * 2)`, ()))
  | None => style
  }

  let style = switch config.aspectRatio {
  | Some(ratio) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~aspectRatio=ratio, ()))
  | None => style
  }
...
let quadrantCellStyle = (position: quadrant): ReactDOM.Style.t => {
  ReactDOM.Style.make(~gridArea=quadrantToArea(position), ())
}
...
let dashboardContainerStyle = (config: dashboardConfig): ReactDOM.Style.t => {
...
let dashboardAreaStyle = (area: dashboardArea): ReactDOM.Style.t => {
  ReactDOM.Style.make(~gridArea=dashboardAreaToString(area), ())
}
...
let flexRow = (~gap: cssLength=Px(8), ~justify: justifyContent=FlexStart, ~align: alignItems=AlignCenter, ()): ReactDOM.Style.t => {
  flexContainerStyle({
    ...defaultFlexConfig,
    direction: Some(Row),
    gap: Some(gap),
...
let flexColumn = (~gap: cssLength=Px(8), ~justify: justifyContent=FlexStart, ~align: alignItems=AlignStretch, ()): ReactDOM.Style.t => {
  flexContainerStyle({
    ...defaultFlexConfig,
...
let equalColumns = (~count: int, ~gap: cssLength=Px(16), ()): ReactDOM.Style.t => {
  gridContainerStyle({
    ...defaultGridConfig,
...
let autoFitGrid = (~minWidth: cssLength, ~gap: cssLength=Px(16), ()): ReactDOM.Style.t => {
  gridContainerStyle({
    ...defaultGridConfig,
...
let spaceBetweenRow = (~align: alignItems=AlignCenter, ()): ReactDOM.Style.t => {
  flexContainerStyle({
    ...defaultFlexConfig,
    direction: Some(Row),
...
let combineStyles = (styles: array<ReactDOM.Style.t>): ReactDOM.Style.t => {
  styles->Belt.Array.reduce(ReactDOM.Style.make(), (acc, s) => ReactDOM.Style.combine(acc, s))
}

@ocaml.doc("Add width and height to a style")
let withSize = (style: ReactDOM.Style.t, ~width: option<cssLength>=?, ~height: option<cssLength>=?, ()): ReactDOM.Style.t => {
  let style = switch width {
  | Some(w) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~width=lengthToString(w), ()))
  | None => style
  }
  switch height {
  | Some(h) => ReactDOM.Style.combine(style, ReactDOM.Style.make(~height=lengthToString(h), ()))
  | None => style
  }
}

@ocaml.doc("Add padding to a style")
let withPadding = (style: ReactDOM.Style.t, padding: cssLength): ReactDOM.Style.t => {
  ReactDOM.Style.combine(style, ReactDOM.Style.make(~padding=lengthToString(padding), ()))
}

@ocaml.doc("Add margin to a style")
let withMargin = (style: ReactDOM.Style.t, margin: cssLength): ReactDOM.Style.t => {
  ReactDOM.Style.combine(style, ReactDOM.Style.make(~margin=lengthToString(margin), ()))
}

@ocaml.doc("Add background color to a style")
let withBackground = (style: ReactDOM.Style.t, color: string): ReactDOM.Style.t => {
  ReactDOM.Style.combine(style, ReactDOM.Style.make(~backgroundColor=color, ()))
}

@ocaml.doc("Add border radius to a style")
let withBorderRadius = (style: ReactDOM.Style.t, radius: cssLength): ReactDOM.Style.t => {
  ReactDOM.Style.combine(style, ReactDOM.Style.make(~borderRadius=lengthToString(radius), ()))
}
