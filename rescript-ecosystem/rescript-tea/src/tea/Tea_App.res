...
//
// Tea_App.res — TEA Application — The core TEA runtime.
//
// This module provides the main application loop that ties together
// the Model, Update, View, and Subscriptions.
//
// Standalone version: no SafeMount dependency.
// Uses Tea_Render.querySelector() directly for DOM mounting.
...
/// Default mount selector — configurable per application
let defaultSelector = "#app"

/// Create a standard TEA program.
/// Mounts to the element matching ~mountSelector (default "#app").
let standardProgram = (
  ~init: unit => ('model, Tea_Cmd.t<'msg>),
  ~update: ('model, 'msg) => ('model, Tea_Cmd.t<'msg>),
  ~view: 'model => Tea_Vdom.t<'msg>,
  ~subscriptions: 'model => Tea_Sub.t<'msg>,
  ~mountSelector: string=defaultSelector,
  (),
  ...
  and render = (): unit => {
    ...
    | (None, _) => {
        // No container — find mount point via querySelector
        switch Tea_Render.querySelector(mountSelector) {
        | None => Console.warn(`Mount point not found: ${mountSelector}`)
        | Some(container) => {
            let renderState = Tea_Render.createState(dispatch)
            Tea_Render.render(container, vdom, renderState)
            state.renderState = Some(renderState)
...
let simpleProgram = (
  ...
  standardProgram(~init, ~update, ~view, ~subscriptions=_ => Tea_Sub.none, ~mountSelector, ())
}
