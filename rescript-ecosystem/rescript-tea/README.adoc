...
event handling, and time-travel debugging.
...
* *React integration* - Uses React for rendering, compatible with
existing components
...
Built-in subscriptions: - `+Sub.Time.every(ms, toMsg)+` - Timer -
`+Sub.Keyboard.downs(toMsg)+` - Key down events -
`+Sub.Keyboard.ups(toMsg)+` - Key up events -
`+Sub.Mouse.clicks(toMsg)+` - Mouse clicks - `+Sub.Mouse.moves(toMsg)+`
- Mouse movement - `+Sub.Window.resizes(toMsg)+` - Window resize
