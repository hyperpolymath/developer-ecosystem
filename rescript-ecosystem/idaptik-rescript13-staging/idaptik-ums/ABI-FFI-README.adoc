...
The Universal Modding Studio uses the hyperpolymath standard ABI/FFI/API
architecture:

[width="100%",cols="21%,27%,25%,27%",options="header",]
|===
|Layer |Language |Purpose |Location
|*ABI* |Idris2 |Level data model with dependent type proofs
|`+src/abi/+`

|*FFI* |Zig |C-compatible shared library |`+ffi/zig/+`

|*Headers* |C (generated) |Bridge between ABI and FFI consumers
|`+generated/abi/+`

|*Shell* |Rust (Gossamer) |Desktop app with IPC commands
|`+src-gossamer/+`
...
|Types |DeviceKind, GuardRank, DogBreed, DroneArchetype, AlertLevel,
ItemCondition
...
|Inventory |CableType, AdapterType, ToolType, ModuleType,
ConsumableType, WorldItem
...
All proofs are erased at compile time (0-quantity fields) — zero runtime
cost:

[arabic]
. *InRegistry* — Device IP exists in the device registry
. *GuardsInZones* — All guards reference valid zones
. *DefenceTargetsValid* — Failover/cascade/mirror targets reference real
devices
. *ZonesOrdered* — Zone transitions monotonically increase in X
. *PBXConsistent* — When hasPBX=True, pbxAddr exists in registry
...
The validation function materialises the 5 erased Idris2 proofs at
runtime.
