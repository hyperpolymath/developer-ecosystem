...
Ideas for expanding the network simulation toward Packet Tracer-level
sophistication.
...
* *VLAN tagging (802.1Q)* - Proper VLAN isolation, trunk ports between
switches
...
* *DHCP server* - Dynamic IP assignment (already have toggle, make
functional)
...
The current abstraction level works well for a hacking game - realistic
enough to feel authentic, simple enough to be fun. Add complexity only
where it creates interesting gameplay mechanics.
