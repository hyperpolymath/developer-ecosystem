...
|`+sync+` |Chainguard wolfi (OTP release) |4000 |Elixir/Phoenix
WebSocket server
|===
...
|`+SECRET_KEY_BASE+` |Yes |— |Phoenix secret (64+ bytes,
`+mix phx.gen.secret+`)
...
Both containers use read-only root filesystems with tmpfs mounts for
runtime writable paths (nginx cache, OTP temp files). Base images are
from Chainguard for minimal attack surface.

All builds use the repo root as build context. The `+.containerignore+`
file excludes heavy directories (node_modules, .git, target/, etc.) to
keep context transfers fast.
