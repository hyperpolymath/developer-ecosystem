...
This document is an early design exploration of making `+reanalyze+`
incremental: keeping state between runs and reacting to file changes.

*Note*: The current implementation lives in `+analysis/reactive/+` and
`+analysis/reanalyze/src/Reactive*+` and differs from some details below
(e.g. no `+-parallel+` flag; caching is implemented in OCaml, not via a
C++ `+Marshal_cache+`).
...
*CMT file loading is the dominant cost* because each file requires: 1.
System call to open file 2. Reading marshalled data from disk 3.
Unmarshalling into OCaml heap 4. AST traversal to extract analysis data
...
[arabic]
. *Persistent service* - Stay running and maintain state between
analysis runs
. *File watching* - React to file changes (create/modify/delete)
. *Incremental updates* - Only process changed files
. *Cached results* - Keep processed `+file_data+` in memory
. *Fast iteration* - Sub-10ms response for typical edits

==== Integration via reactive collections

The implementation uses reactive collections and file-backed collections
to cache processed per-file results and propagate changes incrementally.
...
[arabic]
. *Marshal_cache.with_unmarshalled_if_changed*:
...
For 5000 files with average 20KB each: - mmap cache: ~100MB (off-heap,
OS-managed) - file_data cache: ~50MB (on-heap, estimate)
...
[arabic]
. *Add skip-lite dependency* to dune/opam
. *Create wrapper module* `+CmtCache+` that provides:
+
[source,ocaml]
----
...
[arabic]
. *Define file_data type* as the cached result type
. *Create reactive collection* for CMT → file_data mapping
. *Implement delta processing* that only reprocesses changed files

==== Phase 3: Analysis Service

[arabic]
. *File watching integration* (can use fswatch, inotify, or external
watcher)
. *Service loop* that waits for events and re-runs analysis
. *LSP integration* (optional) for editor support

==== Phase 4: Incremental Merge & Solve (Future)

The current merge and solve phases are relatively fast (22% of time),
but could be made incremental in the future:
...
Here’s a minimal prototype showing how to integrate
`+Reactive_file_collection+`:
...
[arabic]
. *Correctness*: Verify reactive analysis produces same results as batch
. *Performance*: Benchmark incremental updates vs full analysis
. *Edge cases*:
...
[arabic]
. *Build system integration*: How to get file events from rewatch?
. *CMT staleness*: What if build system is still writing CMT files?
. *Multi-project*: How to handle monorepos with multiple rescript.json?
. *Memory limits*: When to evict file_data from cache?
...
The analysis server already has `+DceCommand.ml+`. We can extend it to a
service:
...
Rewatch already watches for file changes. We can extend it to notify the
analysis service:
...
Or via a Unix domain socket/named pipe that the analysis service listens
on.
...
Integrating skip-lite’s reactive collections with reanalyze offers a
path to *39-390x speedup* for incremental analysis. The key insight is
that CMT file loading (77% of current time) can be eliminated for
unchanged files, and the processed file_data can be cached.

The implementation requires: 1. Adding skip-lite as a dependency 2.
Wrapping CMT loading with Marshal_cache (immediate benefit: mmap
caching) 3. Creating reactive collection for file_data (benefit: only
process changed files) 4. Creating a service mode that watches for file
changes (benefit: persistent state)

The merge and solve phases (23% of time) remain unchanged initially, but
could be made incremental in the future for even greater speedups.

=== Next Steps

[arabic]
. *Phase 0*: Add skip-lite as optional dependency (behind a feature
flag)
. *Phase 1*: Replace `+Cmt_format.read_cmt+` with `+Marshal_cache+`
wrapper
. *Phase 2*: Benchmark improvement from mmap caching alone
. *Phase 3*: Implement `+Reactive_file_collection+` for file_data
. *Phase 4*: Create analysis service with file watching
. *Phase 5*: Integrate with rewatch for automatic updates
