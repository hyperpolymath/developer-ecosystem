...
In words: `+iter+` expects as first parameter a function that: makes
progress when called, and let `+S+` bet the set of functions it calls
before making progress. When supplied the first argument, `+iter+` does
not make progress. When supplied the second argument, it makes progress,
and calls functions in set `+S+` before making progress.
