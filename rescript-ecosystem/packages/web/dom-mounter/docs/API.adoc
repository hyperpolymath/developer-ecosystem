...
*Version:* 1.0.0 *Status:* Production Ready with Formal Verification
...
* link:#formal-guarantees[Formal Guarantees]
* link:#api-reference[API Reference]
* link:#architecture[Architecture]
* link:#usage-examples[Usage Examples]
* link:#error-handling[Error Handling]
* link:#testing[Testing]
* link:#formal-verification[Formal Verification]

'''''

=== Introduction

SafeDOM is a formally verified DOM mounting library that provides
*mathematical guarantees* of safety using dependent types (Idris2) and
memory-safe implementation (Zig).

==== Why SafeDOM?

Traditional DOM manipulation is unsafe: - ❌ Null pointer dereferences
(`+querySelector+` returns `+null+`) - ❌ Unbounded string inputs (XSS,
memory exhaustion) - ❌ Runtime errors from invalid selectors - ❌
...
*Guarantee*: DOM element handles cannot be null. Attempting to construct
a null element is a *compile-time type error*.
...
*Guarantee*: CSS selectors are proven to be: - Non-empty (minimum 1
character) - Within bounds (maximum 255 characters) - Memory-safe (no
...
*Guarantee*: HTML content is proven to be: - Within size limit (maximum
1MB) - Tag-balanced (validated before mounting) - Memory-safe (no
allocation failures)
...
*Guarantee*: Memory layouts match C ABI across all platforms with
mathematical proof.
...
*Validation Rules:* - ✓ Length: 1-255 characters - ✓ Characters:
Alphanumeric, `+-+`, `+_+`, `+#+`, `+.+`, `++`, `+[]+`, `+()+`, `+:+`,
`+>+`, `+~+`, `+++`, `+=+` - ✗ Empty strings - ✗ Strings exceeding 255
characters - ✗ Invalid CSS characters (`+<+`, `+>+`, `+{+`, `+}+`, etc.)

*Returns:* - `+Ok(validated)+` if selector is valid (with compile-time
proof) - `+Error(message)+` if validation fails
...
*Validation Rules:* - ✓ Size: 0-1048576 bytes (0-1MB) - ✓ Balanced tags:
`+(openTags - selfClosing) == closeTags+` - ✗ Content exceeding 1MB - ✗
Unbalanced HTML tags

*Returns:* - `+Ok(validated)+` if HTML is valid (with compile-time
proof) - `+Error(message)+` if validation fails
...
*Preconditions (proven at compile-time):* 1. Selector is valid (type
system guarantees) 2. HTML is valid (type system guarantees) 3. No null
pointers possible (type system guarantees)
...
[arabic]
. *Type Safety*: All operations type-check with dependent types
. *Bounds Safety*: String lengths proven within limits
. *Memory Safety*: No buffer overflows possible
. *Null Safety*: No null pointer dereferences possible
. *Platform Safety*: Same ABI across all platforms

==== Verification Evidence

Located in `+src/abi/+`: - `+Types.idr+`: Type-level proofs -
`+Layout.idr+`: Memory layout proofs - `+Foreign.idr+`: FFI safety
proofs - `+SafeDOM.idr+`: Main verification module
...
[arabic]
. *Validate once, reuse validated values:*
+
[source,rescript]
----
// ✓ GOOD
...
----
. *Pre-validate at module initialization:*
+
[source,rescript]
----
// At module scope
...
SafeDOM focuses on *safe mounting only*. For full UI frameworks, use TEA
(The Elm Architecture) with SafeDOM for mounting.
