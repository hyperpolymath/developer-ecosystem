...
")
let step = (~update: ('msg, 'model) => ('model, Tea_Cmd.t<'msg>), ~model: 'model, ~msg: 'msg): step<
  'model,
  'msg,
> => {
  let (newModel, cmd) = update(msg, model)
...
let steps = (
  ...
  let (_, allSteps) = Belt.Array.reduce(msgs, (initialModel, [initialStep]), (
    (model, steps),
    msg,
  ) => {
    let (newModel, cmd) = update(msg, model)
...
let expectModel = (model: 'model, predicate: 'model => bool, message: string): unit => {
  if !predicate(model) {
    Exn.raiseError(`Assertion failed: ${message}`)
  }
}
