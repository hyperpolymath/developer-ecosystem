...
type decoder<'a> = JSON.t => result<'a, decodeError>
...
let int: decoder<int> = json => {
  switch Js.Json.classify(json) {
  | Js.Json.JSONNumber(n) if Math.floor(n) == n => Ok(Belt.Float.toInt(n))
...
let map5 = (
  ...
  json => {
    switch (decoderA(json), decoderB(json), decoderC(json), decoderD(json), decoderE(json)) {
...
let decodeValue = (decoder: decoder<'a>, json: JSON.t): result<'a, decodeError> => {
  decoder(json)
}
...
let decodeString = (decoder: decoder<'a>, str: string): result<'a, decodeError> => {
  ...
  | _ => Error(Failure("invalid JSON", JSON.Encode.null))
  }
}
