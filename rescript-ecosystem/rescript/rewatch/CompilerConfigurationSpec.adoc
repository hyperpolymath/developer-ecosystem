...
This document contains a list of all config parameters with remarks, and
whether they are already implemented in rewatch. It is based on
https://rescript-lang.org/docs/manual/latest/build-configuration-schema.

[width="100%",cols="19%,20%,51%,^10%",options="header",]
|===
...
|js-post-build |Js-Post-Build |Path respects `+in-source+` setting;
stdout/stderr are logged |[x]
...
An object of feature flags to enable experimental compiler behavior.
Only supported by Rewatch.
...
[width="100%",cols="15%,14%,52%,^19%",options="header",]
|===
|Parameter |JSON type |Remark |Implemented?
...
* `+in-source: true+` → path next to the source file (e.g.,
`+src/Foo.js+`)
* `+in-source: false+` → path in `+lib/<module>/+` directory (e.g.,
`+lib/es6/src/Foo.mjs+`)

The command runs with the same working directory as the rewatch process
...
What is this even for? The spec says it is not even implemented in
ReScript. Likely to be removed.
