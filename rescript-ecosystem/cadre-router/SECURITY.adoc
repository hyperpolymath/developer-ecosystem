...
* link:#type-safety-guarantees[Type Safety Guarantees]
...
[arabic]
. Navigate to
https://github.com/hyperpolymath/cadre-router/security/advisories/new[Report
a Vulnerability]
...
This library provides *compile-time type safety* for routing through
ReScript’s type system:
...
[arabic]
. *ReScript Types* - Compile-time route validation
. *Parser Combinators* - Structured input validation
. *Variant Routes* - Impossible to construct invalid routes
. *Type-Safe Parameters* - No string-based route parameters

'''''

=== Cryptographic Requirements

When implementing cryptographic features (e.g., signed routes, auth
tokens in URLs):
...
* *Use Case:* Memorable route IDs (e.g., "`Gigantic-Giraffe-7`" for
journey IDs)
...
https://github.com/hyperpolymath/cadre-router/security/advisories[cadre-router/security/advisories]
...
_Thank you for helping keep cadre-router and its users safe through type
safety._ 🛡️
