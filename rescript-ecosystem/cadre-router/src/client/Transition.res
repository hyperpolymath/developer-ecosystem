...
// CssTransition offloads animation to the browser's CSS engine instead
// of driving it frame-by-frame via requestAnimationFrame. This has
// several advantages:
//
//   1. GPU-accelerated: CSS transforms/opacity run on the compositor
//      thread, never blocking the main thread
//   2. Power-efficient: browser can batch and optimise transitions
//   3. Simpler: no manual easing/timing logic in JS
//   4. Interruptible: CSS transitions can be cancelled mid-flight
//      by simply removing the transition class
//
// The pattern: apply a CSS class that triggers a transition, listen
// for the `transitionend` event, then complete the route transition.
// This keeps Transition.res as the state manager while delegating
// the visual animation entirely to CSS.
//
// Inspired by cadre-router architecture discussion (2026-02-28) and
// the observation that requestAnimationFrame-driven animations are
// overkill for simple route transitions like fades and slides.
module CssTransition = {
  // Apply a CSS class to an element to trigger a CSS transition,
  // and listen for transitionend to complete the route change.
  //
  // Usage:
  ...
  @send external removeEventListener: (Dom.element, string, 'a => unit) => unit = "removeEventListener"
  @send external addClassName: (Dom.element, string) => unit = "add"
  @send external removeClassName: (Dom.element, string) => unit = "remove"
  @get external classList: Dom.element => 'a = "classList"

  // Helper to add/remove classes via classList
  let addClass = (el: Dom.element, className: string): unit => {
    el->classList->addClassName(className)
  }

  let removeClass = (el: Dom.element, className: string): unit => {
    el->classList->removeClassName(className)
  }

  // Animate a route transition using CSS classes.
  // Returns an unsubscribe function to cancel the transition.
  let animate = (
    manager: t<'route>,
    newRoute: 'route,
    ~element: Dom.element,
    ~exitClass: string="route-exit",
    ~enterClass: string="route-enter",
    ...
    addClass(element, exitClass)

    // Force reflow then add active class to trigger CSS transition
    let _ = Js.Math.random()  // Force evaluation order
    addClass(element, activeClass)

    let exitHandler = ref(_ => ())

    exitHandler :=
      (_event => {
        if !cancelled.contents {
          element->removeEventListener("transitionend", exitHandler.contents)
          removeClass(element, exitClass)
          removeClass(element, activeClass)

          // Phase 2: Enter animation
          addClass(element, enterClass)
          let _ = Js.Math.random()
          addClass(element, activeClass)

          let enterHandler = ref(_ => ())
          enterHandler :=
            (_event => {
              if !cancelled.contents {
                element->removeEventListener("transitionend", enterHandler.contents)
          ...
          element->addEventListener("transitionend", enterHandler.contents)
        }
      })

    element->addEventListener("transitionend", exitHandler.contents)
