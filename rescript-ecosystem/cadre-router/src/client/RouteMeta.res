...
// === Optimised Auth Checking ===

// Pre-computed role set for O(1) lookups. When a user session is
// established, build the set once with `makeRoleSet(userRoles)` and
// reuse it for every route check. This avoids the O(R*U) nested
// array scan in `checkAuth` (where R = required roles, U = user roles)
// and replaces it with O(R) Set lookups.
//
// For typical apps with 5-20 roles, the difference is marginal, but
// for fine-grained RBAC with hundreds of permissions this is significant.
//
// Inspired by http-capability-gateway's ETS-based O(1) permission
// lookups and aerie's verb governance trie.
let makeRoleSet = (roles: array<string>): Belt.Set.String.t => {
  Belt.Set.String.fromArray(roles)
}

// checkAuthFast uses a pre-built Set for O(1) role membership checks
// instead of the O(R*U) nested array scan in checkAuth.
//
// Usage:
//   let userRoleSet = RouteMeta.makeRoleSet(userRoles)
//   let result = RouteMeta.checkAuthFast(routeWithMeta, ~isLoggedIn, ~userRoleSet)
let checkAuthFast = (
  routeWithMeta: routeWithMeta<'route, 'custom>,
  ~isLoggedIn: bool,
  ~userRoleSet: Belt.Set.String.t
): authResult<'route> => {
  let {route, meta} = routeWithMeta
  ...
  if !meta.requiresAuth {
    ...
    // O(R) check: each required role is a single Set.has() call
    let hasRole = Belt.Array.some(meta.roles, role =>
      Belt.Set.String.has(userRoleSet, role)
    )
    if hasRole {
      Allowed(route)
    } else {
