...
## Performance Optimisations (v0.3.0)

### Grouped Dispatch with `oneOfGrouped`

The standard `oneOf` tries each parser linearly (O(N) for N routes). For applications
with many routes sharing distinct first segments (e.g., `/api/*`, `/admin/*`, `/user/*`),
`oneOfGrouped` provides near-O(1) dispatch by pre-grouping parsers by their first
...
// Use grouped dispatch:
let parser = Parser.oneOfGrouped([
  ("api",   apiParser),      // matches /api/...
  ("admin", adminParser),    // matches /admin/...
  ("user",  userParser),     // matches /user/...
  ("",      homeParser),     // matches / (empty first segment = root)
])
```

**How it works:** At construction time, parsers are grouped into a `Dict` keyed by
first path segment. When a URL arrives, the first segment is extracted for O(1) dict
lookup, then only parsers within that group are tried linearly. For N routes with K
unique first segments, this reduces average case from O(N) to O(N/K).

`RouteBuilder.oneOfGrouped` provides the same optimisation for bidirectional routes,
accepting a `~keyFromUrl` function for flexible grouping:

```rescript
let router = RouteBuilder.oneOfGrouped(
  [("api", apiBuilder), ("admin", adminBuilder), ("user", userBuilder)],
...
### Set-Based Auth Checking with `checkAuthFast`

The standard `checkAuth` uses nested `Belt.Array.some` loops for role checking (O(R*U)
where R = required roles, U = user roles). For applications with fine-grained RBAC,
...
// O(R) instead of O(R*U) — each role is a single Set.has() call
let result = RouteMeta.checkAuthFast(
  routeWithMeta,
  ~isLoggedIn=true,
  ~userRoleSet,
)
...
### Async Navigation Guards with `guardedPushAsync`

The standard `guardedPush` only runs synchronous guards, ignoring any `asyncGuards`
in the config. This means routes protected by async guards (e.g., checking JWT validity
against a refresh endpoint) would incorrectly allow navigation.

`guardedPushAsync` runs the full guard chain — sync first, then async — and only
...
let config = {
  ...
  asyncGuards: [tokenRefreshGuard, featureFlagGuard],
}

// Deferred navigation — waits for async guards before navigating
let _ = await Tea_Guards.guardedPushAsync(config, targetUrl)
```

### CSS Transition Delegation

The standard `Animation.animate` drives transitions frame-by-frame via
`requestAnimationFrame`. For simple route transitions (fades, slides),
...
// In your CSS:
// .route-exit         { opacity: 1; transition: opacity 200ms ease-out; }
// .route-exit.active  { opacity: 0; }
// .route-enter        { opacity: 0; transition: opacity 200ms ease-in; }
// .route-enter.active { opacity: 1; }
```

**Advantages over rAF-driven animation:**
- GPU-accelerated (compositor thread, never blocks main thread)
- Power-efficient (browser batches and optimises)
- Simpler (no manual easing/timing logic in JS)
- Interruptible (remove class to cancel mid-flight)
...
- **Aerie (dogfooding)**: Verb governance and trie-based routing inspired these optimisations
- **http-capability-gateway**: ETS-based O(1) lookups, policy gate pattern
