...
*Version:* 0.2.0 *Status:* Production Ready *License:* PMPL-1.0-or-later
...
* link:#parser-combinators[Parser Combinators]
* link:#navigation[Navigation]
* link:#tea-integration[TEA Integration]
* link:#examples[Examples]
* link:#best-practices[Best Practices]
* link:#migration-guide[Migration Guide]

'''''

=== Introduction

Cadre Router is a *type-safe routing library* for ReScript applications
that provides:
...
Every route can be: - *Parsed from URL*: `+string → route+` -
...
The standard `+oneOf+` tries each parser linearly (O(N) for N routes).
For applications with many routes sharing distinct first segments (e.g.,
`+/api/*+`, `+/admin/*+`, `+/user/*+`), `+oneOfGrouped+` provides
near-O(1) dispatch by pre-grouping parsers by their first literal path
segment.
...
*How it works:* At construction time, parsers are grouped into a
`+Dict+` keyed by first path segment. When a URL arrives, the first
segment is extracted for O(1) dict lookup, then only parsers within that
group are tried linearly. For N routes with K unique first segments,
this reduces average case from O(N) to O(N/K).

`+RouteBuilder.oneOfGrouped+` provides the same optimisation for
bidirectional routes, accepting a `+~keyFromUrl+` function for flexible
grouping:
...
The standard `+checkAuth+` uses nested `+Belt.Array.some+` loops for
role checking (O(R*U) where R = required roles, U = user roles). For
applications with fine-grained RBAC, `+checkAuthFast+` uses a pre-built
`+Belt.Set.String+` for O(1) membership checks:
...
The standard `+guardedPush+` only runs synchronous guards, ignoring any
`+asyncGuards+` in the config. This means routes protected by async
guards (e.g., checking JWT validity against a refresh endpoint) would
incorrectly allow navigation.

`+guardedPushAsync+` runs the full guard chain — sync first, then async
— and only navigates after ALL guards pass:
...
*Advantages over rAF-driven animation:* - GPU-accelerated (compositor
thread, never blocks main thread) - Power-efficient (browser batches and
optimises) - Simpler (no manual easing/timing logic in JS) -
...
* *Aerie (dogfooding)*: Verb governance and trie-based routing inspired
these optimisations
