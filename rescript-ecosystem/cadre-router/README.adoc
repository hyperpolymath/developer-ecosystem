...
`Parser.oneOfGrouped` and `RouteBuilder.oneOfGrouped` pre-group routes by first
path segment for O(1) dispatch instead of O(N) linear scanning:

[source,rescript]
----
// O(1) dispatch by first segment, then linear within group
let parser = Parser.oneOfGrouped([
  ("api",   apiParser),
  ("admin", adminParser),
  ("user",  userParser),
  ("",      homeParser),   // root/catch-all
])
----

=== Set-Based Auth

`RouteMeta.checkAuthFast` uses a pre-built `Belt.Set.String` for O(1) role
membership checks instead of O(R*U) nested array scans:

[source,rescript]
----
let userRoleSet = RouteMeta.makeRoleSet(userRoles)
let result = RouteMeta.checkAuthFast(routeWithMeta, ~isLoggedIn=true, ~userRoleSet)
----

=== Async Navigation Guards

`Tea_Guards.guardedPushAsync` runs both sync and async guards before navigating,
closing the gap where `guardedPush` ignored async guards:

[source,rescript]
----
let _ = await Tea_Guards.guardedPushAsync(guardConfig, targetUrl)
----

=== CSS Transition Delegation

`Transition.CssTransition.animate` offloads route animations to CSS instead of
driving them frame-by-frame via `requestAnimationFrame`:

[source,rescript]
----
let cancel = Transition.CssTransition.animate(
  manager, newRoute, ~element=container, ~eq=routeEq,
)
----

See link:docs/API_GUIDE.md[API Guide § Performance Optimisations] for details.
