...
= Developer Ecosystem
image:https://img.shields.io/badge/OpenSSF-Best_Practices-green?logo=openssourcesecurity[OpenSSF Best Practices,link="https://www.bestpractices.dev/en/projects/new?repo_url=https://github.com/hyperpolymath/developer-ecosystem"]
...
│       ├── rescript-evangeliser/  # Nextgen languages evangeliser (AffineScript flagship, ReScript legacy)
...
A pattern detection engine and educational toolkit that teaches developers next-generation type-safe languages (AffineScript flagship, ReScript legacy) through progressive code transformation — without shame. Previously ReScript-only; now multi-target with AffineScript as the go-forward language (2026-05-25 policy).
