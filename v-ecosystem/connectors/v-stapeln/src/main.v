...
//
// V-stapeln — V-lang connector for stapeln API.
...
pub struct Config {
pub mut:
	base_url string = 'http://localhost:4010'
}
...
pub fn (c &Client) health() !string {
	resp := http.get('${c.config.base_url}/health') or {
		return error('stapeln unreachable: ${err}')
	}
	if resp.status_code != 200 {
		return error('stapeln unhealthy: HTTP ${resp.status_code}')
	}
	return resp.body
}

// Groove discovery — scan known ports for stapeln
pub fn discover(ports ...int) ?&Client {
	scan_ports := if ports.len > 0 { ports } else { [4010] }
...
pub:
	service   string   = 'stapeln'
	version   string   = '0.1.0'
	api_types []string = ['rest', 'grpc', 'graphql']
	endpoints []string = ['health', 'stacks', 'security', 'pipeline', 'auth']
}
