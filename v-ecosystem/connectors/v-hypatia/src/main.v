...
//
// V-hypatia — V-lang connector for hypatia API.
...
pub fn (c &Client) health() !string {
	resp := http.get('${c.config.base_url}/health') or {
		return error('hypatia unreachable: ${err}')
	}
	if resp.status_code != 200 {
		return error('hypatia unhealthy: HTTP ${resp.status_code}')
	}
	return resp.body
}

// Groove discovery — scan known ports for hypatia
pub fn discover(ports ...int) ?&Client {
	scan_ports := if ports.len > 0 { ports } else { [9090] }
...
pub:
	service   string   = 'hypatia'
	version   string   = '0.1.0'
	api_types []string = ['rest', 'grpc', 'graphql']
	endpoints []string = ['health', 'scan', 'dispatch', 'confidence', 'patterns']
}
