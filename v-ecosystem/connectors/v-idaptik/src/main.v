...
//
// V-idaptik — V-lang connector for idaptik API.
...
pub struct Config {
pub mut:
	base_url string = 'http://localhost:4030'
}
...
pub fn (c &Client) health() !string {
	resp := http.get('${c.config.base_url}/health') or {
		return error('idaptik unreachable: ${err}')
	}
	if resp.status_code != 200 {
		return error('idaptik unhealthy: HTTP ${resp.status_code}')
	}
	return resp.body
}

// Groove discovery — scan known ports for idaptik
pub fn discover(ports ...int) ?&Client {
	scan_ports := if ports.len > 0 { ports } else { [4030] }
...
pub:
	service   string   = 'idaptik'
	version   string   = '0.1.0'
	api_types []string = ['rest', 'grpc', 'graphql']
	endpoints []string = ['health', 'sync', 'levels', 'state']
}
