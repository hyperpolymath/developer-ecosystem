...
All placeholders in this template follow the `v-benchmarks` pattern.
...
sed -i 's/v-benchmarks/MY_PROJECT/g' $(grep -rl 'v-benchmarks' .)
...
| `v-benchmarks` | One-line description | `A tool for X` | flake.nix |
| `v-benchmarks` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-benchmarks` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-benchmarks` markers -- they are TODO markers
