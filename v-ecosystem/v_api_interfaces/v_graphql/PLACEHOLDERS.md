...
All placeholders in this template follow the `v-graphql` pattern.
...
sed -i 's/v-graphql/MY_PROJECT/g' $(grep -rl 'v-graphql' .)
...
| `v-graphql` | One-line description | `A tool for X` | flake.nix |
| `v-graphql` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-graphql` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-graphql` markers -- they are TODO markers
