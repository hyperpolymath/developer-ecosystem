...
All placeholders in this template follow the `+v-rest+` pattern. After
cloning, replace them with your project-specific values.
...
|`+v-rest+` |Uppercase identifier (for Idris2 modules, C macros)
...
|`+v-rest+` |One-line project description
...
These are *not* standard `+v-rest+` markers – they are TODO markers that
must be replaced with project-specific content before release. They mark
