...
All placeholders in this template follow the `v-validator` pattern.
...
sed -i 's/v-validator/MY_PROJECT/g' $(grep -rl 'v-validator' .)
...
| `v-validator` | One-line description | `A tool for X` | flake.nix |
| `v-validator` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-validator` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-validator` markers -- they are TODO markers
