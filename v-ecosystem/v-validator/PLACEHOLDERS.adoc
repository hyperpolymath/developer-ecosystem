...
|===
|Placeholder |Description |Example |Files
|`+Developer Ecosystem+` |Human-readable project name |`+My Project+`
...
|`+v-validator+` |Uppercase identifier (for Idris2 modules, C macros)
...
|`+developer-ecosystem+` |Repository name (slug) |`+my-project+`
...
|`+v-validator+` |One-line project description
...
that must be replaced with project-specific content before release. They
mark sections where the developer (or AI) must fill in:
