...
All placeholders in this template follow the `v-telemetry` pattern.
...
sed -i 's/v-telemetry/MY_PROJECT/g' $(grep -rl 'v-telemetry' .)
...
| `v-telemetry` | One-line description | `A tool for X` | flake.nix |
| `v-telemetry` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-telemetry` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-telemetry` markers -- they are TODO markers
