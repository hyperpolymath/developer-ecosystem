...
All placeholders in this template follow the `v-zig-ffi` pattern.
...
sed -i 's/v-zig-ffi/MY_PROJECT/g' $(grep -rl 'v-zig-ffi' .)
...
| `v-zig-ffi` | One-line description | `A tool for X` | flake.nix |
| `v-zig-ffi` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-zig-ffi` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-zig-ffi` markers -- they are TODO markers
