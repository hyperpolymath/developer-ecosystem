...
This binding was previously part of the hyperpolymath `verification-ecosystem`
repo under `proven/bindings/v/`. It was moved here as part of the V-lang
community transfer (2026-04-17). The hyperpolymath estate is migrating all
FFI bindings to Zig; this V binding is complete and functional, and is offered
to the V community as-is.

Zig and Rust alternative bindings for the same library exist in
`proven/bindings/zig/` and `proven/bindings/rust/` in the upstream repo.

== Licence

PMPL-1.0-or-later.

MPL-2.0 is the automatic legal fallback until PMPL achieves formal legal
recognition. If your V package registry requires an OSI-approved identifier,
use `MPL-2.0`.
