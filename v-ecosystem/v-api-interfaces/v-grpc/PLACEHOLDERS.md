...
All placeholders in this template follow the `v-grpc` pattern.
...
sed -i 's/v-grpc/MY_PROJECT/g' $(grep -rl 'v-grpc' .)
...
| `v-grpc` | One-line description | `A tool for X` | flake.nix |
| `v-grpc` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-grpc` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-grpc` markers -- they are TODO markers
