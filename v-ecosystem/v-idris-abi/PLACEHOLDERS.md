...
All placeholders in this template follow the `v-idris-abi` pattern.
...
sed -i 's/v-idris-abi/MY_PROJECT/g' $(grep -rl 'v-idris-abi' .)
...
| `v-idris-abi` | One-line description | `A tool for X` | flake.nix |
| `v-idris-abi` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
...
| `v-idris-abi` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
...
These are **not** standard `v-idris-abi` markers -- they are TODO markers
