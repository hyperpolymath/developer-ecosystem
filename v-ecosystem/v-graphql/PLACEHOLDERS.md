...
sed -i 's/v-graphql/my-project/g' $(grep -rl 'v-graphql' .)
sed -i 's/V_GRAPHQL/MY_PROJECT/g' $(grep -rl 'V_GRAPHQL' .)
...
sed -i "s/2026-03-13/$(date +%Y-%m-%d)/g" $(grep -rl '2026-03-13' .)
```
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `v-graphql` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `V-lang GraphQL implementation and schema generation with Idris2 ABI proofs and Zig FFI` | One-line description | `A tool for X` | flake.nix |
| `V_GRAPHQL` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, ffi/zig/*.zig |
| `v-graphql` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `2026-03-13` | Current date (ISO) | `2026-02-14` | STATE.a2ml, MAINTAINERS.md |
| `2026-03-13` | Last updated date | `2026-02-14` | TOPOLOGY.md, THREAT-MODEL.md |
...
| `maintainer` | Conduct committee name | `Code of Conduct Committee` | CODE_OF_CONDUCT.md |
| `72 hours` | SLA for initial response | `48 hours` | CODE_OF_CONDUCT.md |
...
| `V-lang GraphQL implementation and schema generation` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
